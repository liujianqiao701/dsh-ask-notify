...
Write-Host '    0.1.2+ a ui-session service publishes it instead. dsh-ask-notify 1.0.2+'
Write-Host '    reads BOTH, so either generation works. Builds up to 1.0.1 pinned'
...
if ($harnessRoots.Count -eq 0) {
  ...
  foreach ($root in $harnessRoots) {
    ...
    Write-Host '                                      1.0.2+ reads whichever is there; builds <= 1.0.1 needed the runtime one'
...
if ($hit.Count -gt 0) {
  ...
  Write-Host '                 used:"uiSession.status" / "uiSession.pending" / "list" = reading the wait state;'
