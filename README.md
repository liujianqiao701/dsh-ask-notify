...
## 发版清单（维护者）—— **两处都要发，只推 GitHub 不够**

> ⚠️ 市场的收录条目里写着 `"npm": "dsh-ask-notify"`，而 `dshmarket/lib/sources.js` 的
> `installTargetFor()` 明确规定 **npm 包优先**（注释原文 *"Repo-verified npm packages **win**"*），
> 只有条目**没有** `npm` 字段时才回落到 `github:`。所以 **npm 不发布，从市场「发现 / 搜索」页
> 安装的人拿到的就是 npm 上的旧版**。2026-09-24 实测确认：npm 停在 1.0.0 期间，搜索页显示
> `v1.0.0`，装下去的也正是那份"只认一个取数源、`inject` 还钉着死引用"的旧代码 ——
> 这就是"装完没效果"。

| 步骤 | 命令 / 动作 | 为什么 |
| --- | --- | --- |
| 1 | 改 `package.json` 的 `version` | 后面两处发布都认它 |
| 2 | **`npm publish`**（在包目录执行） | 市场的**安装源优先取 npm**；不发 npm = 搜索页装到旧版 |
| 3 | `git push` 到 `main` | `github:` 源（本机现在这条）按 HEAD 更新；市场也靠 git ref 判断"有没有新版" |
| 4 | 打 Release（可选） | 市场卡片的「更新内容」会读它 |
| 5 | 等收录库刷新 | 搜索页显示的是**收录库**的版本，收录库版本号**取自 npm**，**异步刷新**、不是瞬时 |

**两个版本号别搞混**：

| 界面 | 显示 | 来源 |
| --- | --- | --- |
| 市场「发现 / 搜索」卡片 | `v1.0.0` | 收录库的 `version` 字段 ← **取自 npm** |
| 市场「已安装」卡片 | `v1.0.4` | **实际装的包**的 `package.json` |

> 已安装卡片上出现「**换用 npm 版本**」按钮，说明"装的源"与"收录源"不一致 ——
> 在两边版本一致之前**别点它**，点了会降级到 npm 上的版本。
